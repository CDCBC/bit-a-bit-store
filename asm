#!/usr/bin/bash
# This script helps with working with aarch64 assembly.
...
if [[ ! " ${commands[*]} " =~ " ${comm} " ]] ; then 
    printf "${b}asm${g} [run, exe, dis, gdb, help]\n"
    exit 1
fi
...
function debug {
    if [[ ! "$file" ]] ; then
        printf "${b}asm${g} gdb${w} file.s\n"
        exit 1
    fi

    if [[ $(whoami) == "osboxes" ]]; then
        # if the script is being executed on the VM provided by the processor
        # then use the appropriate programs
        pkill gdb-multiarch
    ...
    aarch64-linux-gnu-gcc -static -o $no_ext -g $file
    $TERM --hold -e qemu-aarch64 -singlestep -g 1234 $no_ext &
...
function disassembly {
    ...
    if [[ ! " ${dis_flags[*]} " =~ " ${flag} " ]] ; then 
        printf "${b}asm ${g}dis${y} [-s, -t, -d, -D]${w} file.s\n"
        exit 1
    fi
...
function assembly_and_execute {
    if [[ ! "$file" ]] ; then
        printf "${b}asm${g} run${w} file.s\n"
        exit 1
    fi
